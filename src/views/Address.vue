<template>
    <div>
      ...
      <nav-bread>
        <span style="fontSize:25px">收货地址</span>
      </nav-bread>
              ...
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    <dl>
                    <dt> <span>姓名：</span> {{item.userName}}</dt>
                      <dd class="address"><span>地址：</span>{{item.province+item.city+item.area+item.streetName}}</dd>
                    <dd class="tel"><span>  电话号码：</span>{{item.tel}}</dd>
                    </dl>
      ...
      <modal :mdShow="isAdd" @close="closeModal">
        <p slot="message">
          <!-- <input type="text" v-model="addressId" placeholder="地址编号"> -->
            <!-- <input type="text" :value="currentAddressId" > -->
             <input type="text" v-model="userName" placeholder="用户姓名">
            <v-distpicker  @selected="onSelected"></v-distpicker>

          <input type="text" v-model="tel" placeholder="电话号码">
          <input style="width:230px" type="text" v-model="streetName" placeholder="街道名">
          <span>是否设置为默认地址:</span>
      ...
      this.area=data.area.value
          ...
          init(){
              axios.get("/api/users/addressList").then((response)=>{
                  ...
                  this.currentAddressId=this.addressList.length
        ...
        addAddress(){
          ...
          if(this.city==''||this.province==''||this.area==''){
            alert('省市区不能为空')
            return;
          }
            if(this.tel==null){
          ...
          if(this.streetName==''){
            ...
            axios.post('/api/users/addAddress',{
              addressId:this.currentAddressId+1,
