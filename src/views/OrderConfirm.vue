    ...
    <div class="container">
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          ...
          <!-- <div class="prev-btn-wrap">
            <button class="btn btn--m" @click="saveOrder">保存订单</button>
          </div>-->
          <div class="next-btn-wrap">
            <!-- <button a-class="btn btn--m btn--red" @click="saveOrder">确认并保存订单</button> -->
            <a-button type="danger" @click="saveOrder">确认并保存订单</a-button>
          </div>
        </div>
      </div>
    </div>
      ...
      axios
        ...
        .then(response => {
          ...
          if (res.status == "0") {
            ...
            //     path:'/orderSuccess?orderId='+res.result.orderId
            // })
            // alert('订单保存成功,请前往历史订单中查询')
            this.$router.push("/order");
            this.$message.success("订单保存成功,已跳转至历史订单页面");
          }
        });
    }
    // toAlipay(){
    //     var addressId = this.$route.query.addressId;
    //     axios.post("/api/users/alipay",{
    //        addressId:addressId,
    //       orderTotal:this.orderTotal
    //     }).then((response)=>{
    //         let res = response.data;
